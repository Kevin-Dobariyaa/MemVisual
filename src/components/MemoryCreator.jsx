...
function MemoryCreator({ createMemory, partitionType }) {
  const [blockSize, setBlockSize] = useState(128);
  const [blocks, setBlocks] = useState([]);
  const [noBlock, setNoBlock] = useState(1);
  const [error, setError] = useState(null);
  const [size, setSize] = useState(1024); // Default size for dynamic partition
  ...
  const createMemoryy = () => {
    if (partitionType === "dynamic") {
      if (size <= 0) {
        setError("Memory size must be greater than 0");
        return;
      }
      createMemory({ blocks:[size], noBlock: 1 });
      return;
    }
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 w-64 bg-gray-800 text-white text-sm rounded p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity z-10">
            {partitionType === "static" &&
              "Configure the total number block and block size for static partition."}
            {partitionType === "dynamic" &&
              "Configure the total memory size for dynamic partition."}
          </div>
        </div>
      </div>
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button className="btn" onClick={() => handleAddStatic()}>
